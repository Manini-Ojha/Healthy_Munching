...
router.post("/register", async (req, res) => {
  ...
  if (req.sessionID) {
    await run("UPDATE OR IGNORE cart_items SET owner_id = ? WHERE owner_id = ?", [String(user.id), req.sessionID]);
    await run("DELETE FROM cart_items WHERE owner_id = ?", [req.sessionID]);
  }
